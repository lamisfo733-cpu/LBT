...
function loadStage(stageId) {
    ...
    // Check if this is a special interactive stage
    if (stage.type === 'special') {
        loadSpecialStage(stage, stageProgress);
        return;
    }
    
    // Regular quiz/puzzle stage
    content.innerHTML = `
...
// Load special interactive stages
function loadSpecialStage(stage, stageProgress) {
    const content = document.getElementById('stageContent');
    ...
    content.innerHTML = `
        <div class="challenge-header">
            ...
            <div style="text-align: center; margin: 1rem 0;">
                <span style="color: var(--accent-yellow); font-weight: 700;">
                    ${stageProgress.completed ? '✅ تم الإكمال - ' + stageProgress.score + ' نقطة' : 'جاهز للبدء!'}
                </span>
            </div>
        </div>
        <div id="challengesContainer"></div>
    ...
    // Initialize the appropriate special game
    switch (stage.specialType) {
        case 'codeDebugger':
            initStage8();
            break;
        case 'chemicalLab':
